...
#define RTREE_SEARCH_TEMPLATE                                                  \
    template <class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, \
              int TMAXNODES, int TMINNODES, class VISITOR>
#define RTREE_QUAL \
...
// Fwd decl
class
    RTFileStream;  // File I/O helper class, look below for implementation and notes.
...
class RTree {
   ...
   public:
    ...
    bool Remove(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS],
                const DATATYPE& a_dataId);
    ...
    /// \return Returns the DATATYPE located closest to a_point, 0 if the tree is empty.
    DATATYPE NearestNeighbor(
        const ELEMTYPE a_point[NUMDIMS],
        ELEMTYPE a_squareDistanceCallback(const ELEMTYPE a_point[NUMDIMS],
                                          DATATYPE a_data),
    ...
    class Iterator {
       ...
       private:
        ...
        StackElement m_stack
            [MAX_STACK];  ///< Stack as we are doing iteration instead of recursion
   ...
   protected:
    ...
    template <class VISITOR>
    bool Search(Node* a_node, Rect* a_rect, VISITOR& a_visitor,
                int& a_foundCount) {
        ASSERT(a_node);
...
RTREE_TEMPLATE
DATATYPE RTREE_QUAL::NearestNeighbor(
    const ELEMTYPE a_point[NUMDIMS],
    ELEMTYPE a_squareDistanceCallback(const ELEMTYPE a_point[NUMDIMS],
                                      DATATYPE a_data),
    ELEMTYPE* a_squareDistance) {
    ...
    while (!closestNode || !closestNode->isLeaf) {
        ...
        for (int index = 0; index < node->m_count; ++index) {
            ...
            if (newNode->isLeaf && a_squareDistanceCallback)
                newNode->minDist =
                    a_squareDistanceCallback(a_point, newNode->m_branch.m_data);
            else
                newNode->minDist =
                    this->MinDist(a_point, &(node->m_branch[index].m_rect));
        ...
        while (reInsertList) {
            ...
            for (int index = 0; index < tempNode->m_count; ++index) {
                InsertRect(&(tempNode->m_branch[index].m_rect),
                           tempNode->m_branch[index].m_data, a_root,
                           tempNode->m_level);
            }
...
RTREE_TEMPLATE
void RTREE_QUAL::InsertNNListSorted(std::vector<NNNode*>* nodeList,
                                    NNNode* newNode) {
    typedef typename std::vector<NNNode*>::iterator iterator;
